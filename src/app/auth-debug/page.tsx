...
export default function AuthDebugPage() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      console.log('AuthDebug: Attempting sign in with email:', email)
      await signIn(email, password)
      setResult('Sign in successful!')
      console.log('AuthDebug: Sign in completed')
    } catch (err: any) {
      console.error('AuthDebug: Sign in error:', err)
  ...
  const handleSignOut = async () => {
    setLoading(true)
    try {
      console.log('AuthDebug: Attempting sign out')
      await signOut()
      setResult('Sign out successful!')
      console.log('AuthDebug: Sign out completed')
    } catch (err: any) {
      console.error('AuthDebug: Sign out error:', err)
  ...
  const handleGoToAdmin = () => {
    console.log('AuthDebug: Navigating to admin')
    router.push('/admin')
  }
    ...
    <div className="min-h-screen p-8 bg-gray-50">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded">
          ...
          <p className="text-sm">
            This page helps debug authentication issues. If you're experiencing infinite loading after sign in,
